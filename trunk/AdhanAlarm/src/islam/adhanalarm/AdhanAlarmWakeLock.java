...
public class AdhanAlarmWakeLock {
	...
	private static final long shortWakeTime = 20000; // 20 seconds enough time to complete a "Default Notification"
	...
	static void acquireShort(Context context) {
		release();
		PowerManager powerManager = (PowerManager)context.getSystemService(Context.POWER_SERVICE);

        wakeLock = powerManager.newWakeLock(PowerManager.FULL_WAKE_LOCK | PowerManager.ACQUIRE_CAUSES_WAKEUP | PowerManager.ON_AFTER_RELEASE, "Adhan Alarm Wake Lock");
        wakeLock.acquire(shortWakeTime);
	}
